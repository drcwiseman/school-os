...
import { verifyPassword } from "../src/middleware/auth";
...
async function main() {
  ...
  const admins = await db.execute<{ email: string }>(sql`
    SELECT email FROM platform_admins LIMIT 5
  `);
  console.log("platform_admins rows:", admins.rows.length ? admins.rows : "(none)");

  const pw = await db.execute<{ password_hash: string }>(sql`
  ...
  const ok = await verifyPassword("Platform123!", pw.rows[0].password_hash);
